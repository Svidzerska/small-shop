...
const EditCategories: React.FC = (): JSX.Element => {
    ...
    const chooseCategory = (e: React.MouseEvent<HTMLButtonElement>): void => {
        if (!isEditingCategory) {
            ...
            const checkIsActiveCategory: boolean = activeCategories.includes(
                e.currentTarget.value
            );
            checkIsActiveCategory
                ? setActiveCategories(
                      activeCategories.filter((item) => item !== targetValue)
                  )
    ...
    const editCategory = (
        ...
        const existingCategory: Category = temporaryCategories.find(
            (item) => item.id === id
        )!;
    ...
    const deleteCategory = (
        ...
        dispatch(
            setTemporaryCategories(
                temporaryCategories.filter((item) => item.id !== id)
            )
        );

        const deleteCategory = temporaryCategories.find(
            (item) => item.id === id
        );
        setActiveCategories(
            activeCategories.filter((item) => item !== deleteCategory?.name)
        );
    };
    ...
    const doneCategory = (
        ...
        const secondElementCategory = currentCategories.find(
            (item) => item.name === value && item.id !== id
        );

        if (!secondElementCategory && value !== "") {
            const currentCategory = currentCategories.findIndex(
                (item) => item.id === id
            );
            ...
            const deleteCategory = currentCategories.find(
                (item) => item.id === id
            );
            setActiveCategories(
                activeCategories.filter((item) => item !== deleteCategory?.name)
        ...
        return (
            <>
                {activeCategories.find((item) => item === category.name) &&
                    isEditingCategory &&
                    editedCategory === category.id && (
                        <p className="edit-component__char-amount">
    ...
    const displayCategories: JSX.Element[] = temporaryCategories.map(
        (category: Category) => {
            return (
                <li
                    key={category.id}
